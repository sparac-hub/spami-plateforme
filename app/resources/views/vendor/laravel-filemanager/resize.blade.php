<div class="row">
    ...
    <div class="col-md-4">
        ...
        <table class="table table-compact table-striped">
            ...
            @if ($scaled)
                <tr>
                    <td>{{ __('laravel-filemanager::lfm.resize-ratio') }}</td>
                    <td>{{ number_format($ratio, 2) }}</td>
                </tr>
                <tr>
                    <td>{{ __('laravel-filemanager::lfm.resize-scaled') }}</td>
                    <td>
            ...
            @endif
            <tr>
                <td>{{ __('laravel-filemanager::lfm.resize-old-height') }}</td>
                <td>{{ $original_height }}px</td>
            </tr>
            <tr>
                <td>{{ __('laravel-filemanager::lfm.resize-old-width') }}</td>
                <td>{{ $original_width }}px</td>
            </tr>
            <tr>
                <td>{{ __('laravel-filemanager::lfm.resize-new-height') }}</td>
                <td><span id="height_display"></span></td>
            </tr>
            <tr>
                <td>{{ __('laravel-filemanager::lfm.resize-new-width') }}</td>
                <td><span id="width_display"></span></td>
            </tr>
            </tbody>
        </table>

        <button class="btn btn-primary" onclick="doResize()">{{ __('laravel-filemanager::lfm.btn-resize') }}</button>
        <button class="btn btn-info" onclick="loadItems()">{{ __('laravel-filemanager::lfm.btn-cancel') }}</button>
