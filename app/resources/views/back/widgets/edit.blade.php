...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.widgets.edit') }}
@endsection

@section('content')

    {!! set_box_head(__('breadcrumbs.back.widgets.edit'), false) !!}

    @include('_common.alerts.messages')
    <div class="alert alert-info">
        {{__('og.widgets.widget_edit_info')}}
    </div>
    <form action="{{ route('back.widgets.update', $widget->id) }}" method="post" class="form-create">
                    ...
                    <div class="tab-pane @if($k==0) active @endif" id="tab_1_{{$locale}}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="{{$locale}}[name]"
                                   value="{{ old($locale.'.name', $widget->translateOrNew($locale)->name) }}" required>
                        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="reference" value="{{ old('reference',$widget->reference) }}">
        </div>
            ...
            <select class="form-control" name="placement" required>
                ...
                @foreach(config('widgets.placement') as $placement)
                    <option value="{{ $placement }}"
                            @if(old('placement', $widget->placement) == $placement) selected @endif >
            ...
            <select class="form-control" name="module_id" id="module_id" required>
                ...
                @foreach($modules as $module)
                    <option value="{{$module->id}}"
                            @if($module->id == old('module_id',$widget->module_id)) selected @endif>
                        {{$module->name}}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="type" required>
                ...
                @foreach(config('widgets.type') as $type => $value)
                    <option value="{{ $type }}" @if(old('type', $widget->type) == $type) selected @endif >
                        {{ $value }}
                    </option>
                @endforeach
            </select>
