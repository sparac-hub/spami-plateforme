...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.news_categories.index', $menu_id) }}
@endsection
...
@section('content')
    ...
    {!! set_box_head(__('breadcrumbs.back.news_categories.index'), false) !!}

    @can('back.news_categories.create')
        <a href="{{ route('back.news_categories.create', $where) }}"
    ...
    @can('back.news.index')
        <a href="{{ route('back.news.index', $where) }}"
           class="btn btn-primary">
            News
        </a>
    @endcan
    ...
    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.news_categories.id')}} |
    <input type="checkbox" class="toggle-vis" data-column="order" checked/> {{__('og.news_categories.order')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.slug"
           checked/> {{__('og.news_category_translations.slug')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.name"
           checked/> {{__('og.news_category_translations.name')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.description"
           checked/> {{__('og.news_category_translations.description')}} |
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            <thead>
            <tr>
                <th>{{__('og.news_categories.id')}}</th>
                <th>{{__('og.news_categories.order')}}</th>
                <th>{{__('og.news_category_translations.slug')}}</th>
                <th>{{__('og.news_category_translations.name')}}</th>
                <th>{{__('og.news_category_translations.description')}}</th>
                <th>{{__('og.actions')}}</th>
                {{--
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.news_categories.index', $where) !!}',
                columns: [
                    {data: 'id', name: 'id'},
    ...
    @include('back._common.datatables.toggleStatusJs', [
        'route' => 'back.news_categories.index'
    ])
@stop
