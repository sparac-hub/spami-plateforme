...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.outil_gestion_categories.index', $menu_id) }}
@endsection
...
@section('content')
    ...
    {!! set_box_head(__('breadcrumbs.back.outil_gestion_categories.index'), false) !!}

    @can('back.outil_gestion_categories.create')
        <a href="{{ route('back.outil_gestion_categories.create', $where) }}"
           class="btn btn-primary">
            {{ __('og.button.create') }}
        </a>
    @endcan

    @can('back.outil_gestions.index')
        <a href="{{ route('back.outil_gestions.index', $where) }}"
           class="btn btn-primary">
            Outils de gestion
        </a>
    @endcan
    ...
    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.outil_gestion_categories.id')}} |
    <input type="checkbox" class="toggle-vis" data-column="order"
           checked/> {{__('og.outil_gestion_categories.order')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.slug"
           checked/> {{__('og.outil_gestion_category_translations.slug')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.name"
           checked/> {{__('og.outil_gestion_category_translations.name')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.description"
           checked/> {{__('og.outil_gestion_category_translations.description')}} |
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            <thead>
            <tr>
                <th>{{__('og.outil_gestion_categories.id')}}</th>
                <th>{{__('og.outil_gestion_categories.order')}}</th>
                <th>{{__('og.outil_gestion_category_translations.slug')}}</th>
                <th>{{__('og.outil_gestion_category_translations.name')}}</th>
                <th>{{__('og.outil_gestion_category_translations.description')}}</th>
                <th>{{__('og.actions')}}</th>
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.outil_gestion_categories.index', $where) !!}',
                columns: [
                    {data: 'id', name: 'id'},
    ...
    @include('back._common.datatables.toggleStatusJs', [
        'route' => 'back.outil_gestion_categories.index'
    ])
@stop
