...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.plan_categories.index', $menu_id) }}
@endsection

@section('content')

    {!! set_box_head(__('breadcrumbs.back.plan_categories.index'), false) !!}

    @php($where = request('menu_id') ? ['menu_id' => request('menu_id')] : null)

    @can('back.plan_categories.create')
        <a href="{{ route('back.plan_categories.create', $where) }}"
    ...
    @can('back.plans.index')
        <a href="{{ route('back.plans.index', ['menu_id' => request('menu_id')]) }}"
           class="btn btn-primary">
            Plans items List
        </a>
    @endcan
    ...
    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.plan_categories.id')}} |
    <input type="checkbox" class="toggle-vis" data-column="name"
           checked/> {{__('og.plan_category_translations.name')}} |
    <input type="checkbox" class="toggle-vis" data-column="order" checked/> {{__('og.plan_categories.order')}} |
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            <thead>
            <tr>
                <th>{{__('og.plan_categories.id')}}</th>
                <th>{{__('og.plan_category_translations.name')}}</th>
                <th>{{__('og.plan_categories.order')}}</th>
                <th>{{__('og.actions')}}</th>
            </tr>
            </thead>
        </table>
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.plan_categories.index', $where) !!}',
                columns: [
                    {data: 'id', name: 'id'},
    ...
    @include('back._common.datatables.toggleStatusJs', [
        'route' => 'back.plan_categories.index'
    ])
@stop
