...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.sitemaps.index',$menu_id) }}
@endsection
...
@section('content')
    ...
    {!! set_box_head(__('breadcrumbs.back.sitemaps.index'), false) !!}

    @include('_common.alerts.messages')

    @can('back.sitemaps.create')
        <a href="{{ route('back.sitemaps.create', ['menu_id' => $menu_id]) }}"
    ...
    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.sitemaps.id')}} |
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.sitemaps.index', $where) !!}',
                columns: [
                    {data: 'id', name: 'id'},
