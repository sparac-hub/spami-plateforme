...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.procedure_categories.index', $menu_id) }}
@endsection

@section('content')

    {!! set_box_head(__('breadcrumbs.back.procedure_categories.index'), false) !!}

    @php($where = request('menu_id') ? ['menu_id' => request('menu_id')] : null)

    @can('back.procedure_categories.create')
        <a href="{{ route('back.procedure_categories.create', $where) }}"
    ...
    @can('back.files.index')
        <a href="{{ route('back.procedures.index', ['menu_id' => request('menu_id')]) }}"
           class="btn btn-primary">
            Files items List
        </a>
    @endcan
    ...
    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.procedure_categories.id')}} |
    <input type="checkbox" class="toggle-vis" data-column="name"
           checked/> {{__('og.procedure_category_translations.name')}} |
    <input type="checkbox" class="toggle-vis" data-column="order" checked/> {{__('og.procedure_categories.order')}} |
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            <thead>
            <tr>
                <th>{{__('og.procedure_categories.id')}}</th>
                <th>{{__('og.procedure_category_translations.name')}}</th>
                <th>{{__('og.procedure_categories.order')}}</th>
                <th>{{__('og.actions')}}</th>
            </tr>
            </thead>
        </table>
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.procedure_categories.index', $where) !!}',
                columns: [
                    {data: 'id', name: 'id'},
    ...
    @include('back._common.datatables.toggleStatusJs', [
        'route' => 'back.procedure_categories.index'
    ])
@stop
