...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.programs.create', $menu_id) }}
@endsection

@section('content')

    {!! set_box_head(__('breadcrumbs.back.programs.create'), FALSE) !!}

    @include('_common.alerts.messages')

    <form action="{{ route('back.programs.store') }}" method="post" class="form-create">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="page_id">
                ...
                @foreach($menus as $menu)
                    <option {{ ($menu->id == old('page_id'))?'selected':'' }}
                            value="{{ $menu->id }}">{{ $menu->label }}</option>
                @endforeach
            </select>
        </div>

        @php($aspimsArray = old('aspims')??[])
        <div class="form-group">
                ...
                @if($aspims->isNotEmpty())
                    @foreach($aspims as $aspim)
                        <option {{(in_array($aspim->id, $aspimsArray)?'selected':'')}}
                                value="{{ $aspim->id }}">{{ $aspim->name }}</option>
                    @endforeach
                @endif
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input autocomplete="off" type="text" class="form-control datepicker" id="established_at"
                   name="established_at" value="{{ old('established_at') }}">
        </div>
