...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.media_album_categories.index', $menu_id) }}
@endsection
...
@section('content')
    ...
    {!! set_box_head(__('breadcrumbs.back.media_album_categories.index'), false) !!}

    @can('back.media_album_categories.create')
        <a href="{{ route('back.media_album_categories.create', $where) }}"
    ...
    @can('back.media_albums.index')
        <a href="{{ route('back.media_albums.index', $where) }}"
           class="btn btn-primary">
            Media album
        </a>
    @endcan
    ...
    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.media_album_categories.id')}} |
    <input type="checkbox" class="toggle-vis" data-column="order" checked/> {{__('og.media_album_categories.order')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.slug"
           checked/> {{__('og.media_album_category_translations.slug')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.name"
           checked/> {{__('og.media_album_category_translations.name')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.description"
           checked/> {{__('og.media_album_category_translations.description')}} |
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            <thead>
            <tr>
                <th>{{__('og.media_album_categories.id')}}</th>
                <th>{{__('og.media_album_categories.order')}}</th>
                <th>{{__('og.media_album_category_translations.slug')}}</th>
                <th>{{__('og.media_album_category_translations.name')}}</th>
                <th>{{__('og.media_album_category_translations.description')}}</th>
                <th>{{__('og.actions')}}</th>
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.media_album_categories.index', $where) !!}',
                columns: [
                    {data: 'id', name: 'id'},
    ...
    @include('back._common.datatables.toggleStatusJs', [
        'route' => 'back.media_album_categories.index'
    ])
@stop
