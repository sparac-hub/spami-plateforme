...
@section('content')
    ...
    {!! set_box_head(__('og.box_title.list').'Parameter_groups', false) !!}

    @include('back._common.alerts.messages')


    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.parameter_groups.id')}} |
    <input type="checkbox" class="toggle-vis" data-column="name" checked/> {{__('og.parameter_groups.name')}} |
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            <thead>
            <tr>
                <th>{{__('og.parameter_groups.id')}}</th>
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.parameter_groups.datatables') !!}',
                columns: [
                    {data: 'id', name: 'id'},
                    {data: 'name', name: 'name'},
                    {{--
