...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.training_categories.index', $menu_id) }}
@endsection
...
@section('content')
    ...
    {!! set_box_head(__('breadcrumbs.back.training_categories.index'), false) !!}

    @can('back.training_categories.create')
        <a href="{{ route('back.training_categories.create', $where) }}"
           class="btn btn-primary">
            {{ __('og.button.create') }}
        </a>
    @endcan

    @can('back.trainings.index')
        <a href="{{ route('back.trainings.index', $where) }}"
           class="btn btn-primary">
            Trainings
        </a>
    @endcan
    ...
    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.training_categories.id')}} |
    <input type="checkbox" class="toggle-vis" data-column="order" checked/> {{__('og.training_categories.order')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.slug"
           checked/> {{__('og.training_category_translations.slug')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.name"
           checked/> {{__('og.training_category_translations.name')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.description"
           checked/> {{__('og.training_category_translations.description')}} |
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            <thead>
            <tr>
                <th>{{__('og.training_categories.id')}}</th>
                <th>{{__('og.training_categories.order')}}</th>
                <th>{{__('og.training_category_translations.slug')}}</th>
                <th>{{__('og.training_category_translations.name')}}</th>
                <th>{{__('og.training_category_translations.description')}}</th>
                <th>{{__('og.actions')}}</th>
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.training_categories.index', $where) !!}',
                columns: [
                    {data: 'id', name: 'id'},
    ...
    @include('back._common.datatables.toggleStatusJs', [
         'route' => 'back.training_categories.index'
     ])
@stop
