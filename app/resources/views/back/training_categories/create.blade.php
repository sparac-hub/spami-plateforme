...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.training_categories.create', $menu_id) }}
@endsection

@section('content')

    {!! set_box_head(__('breadcrumbs.back.training_categories.create'), false) !!}

    @include('_common.alerts.messages')

    <form action="{{ route('back.training_categories.store') }}" method="post" class="form-create">
                ...
                <div class="icheck-inline">
                    ...
                    <label>
                        <input type="radio" name="is_active" value="0"
                               @if(old('is_active') == 1) checked @endif class="icheck"> Désactivée
                    </label>
                </div>
            </div>
        </div>
