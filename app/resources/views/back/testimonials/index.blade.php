...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.testimonials.index', $menu_id) }}
@endsection
...
@section('content')
    ...
    {!! set_box_head(__('breadcrumbs.back.testimonials.index'), false) !!}

    @include('_common.alerts.messages')

    @can('back.testimonials.create')
        <a href="{{ route('back.testimonials.create', ['menu_id' => request('menu_id')]) }}"
           class="btn btn-primary">
            {{ __('og.button.create') }}
        </a>
    @endcan

    @can('back.testimonial_categories.index')
        <a href="{{ route('back.testimonial_categories.index', ['menu_id' => request('menu_id')]) }}"
           class="btn btn-primary">
    ...
    <input type="checkbox" class="toggle-vis" data-column="id" checked/> {{__('og.testimonials.id')}} |
    <input type="checkbox" class="toggle-vis" data-column="testimonial_category_id"
           checked/> {{__('og.testimonials.testimonial_category_id')}} |
    <input type="checkbox" class="toggle-vis" data-column="image" checked/> {{__('og.testimonials.image')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.title"
           checked/> {{__('og.testimonial_translations.title')}} |
    <input type="checkbox" class="toggle-vis" data-column="translations.description"
           checked/> {{__('og.testimonial_translations.description')}} |
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="data-table">
            <thead>
            <tr>
                <th>{{__('og.testimonials.id')}}</th>
                <th>{{__('og.testimonials.testimonial_category_id')}}</th>
                <th>{{__('og.testimonials.image')}}</th>
                <th>{{__('og.testimonial_translations.title')}}</th>
                <th>{{__('og.testimonial_translations.description')}}</th>
                <th>{{__('og.actions')}}</th>
            </tr>
            </thead>
        </table>
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('back.testimonials.index', $where) !!}',
                columns: [
                    {data: 'id', name: 'id'},
