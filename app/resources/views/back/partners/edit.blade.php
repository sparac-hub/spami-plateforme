...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.partners.edit', $partner->menu_id) }}
@endsection

@section('content')

    {!! set_box_head(__('breadcrumbs.back.partners.edit'), false) !!}

    @include('_common.alerts.messages')

    <form action="{{ route('back.partners.update', $partner->id) }}" method="post" class="form-create">
                    ...
                    <div class="tab-pane @if($k==0) active @endif" id="tab_1_{{$locale}}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="{{$locale}}[title]"
                                   value="{{ old($locale . '.title', $partner->translateOrNew($locale)->title) }}">
                        </div>

                        <div class="form-group">
                            <label>{{__('og.partner_translations.description')}} </label>
                            <textarea class="form-control" name="{{$locale}}[description]"
                                      rows="3">{{ old($locale . '.description', $partner->translateOrNew($locale)->description) }}</textarea>
                        </div>
                ...
                <div class="icheck-inline">
                    <label>
                        <input type="radio" name="is_active" value="1"
                               @if(old('is_active', $partner->is_active)){{'checked'}}@endif class="icheck"> Activé
                    </label>
                    <label>
                        <input type="radio" name="is_active" value="0"
                               @if(! old('is_active', $partner->is_active)){{'checked'}}@endif class="icheck"> Désactivé
                    </label>
                </div>
            </div>
        </div>
                ...
                @if($partner_categories)
                    @foreach ($partner_categories as $partner_category)
                        <option value="{{ $partner_category->id }}"
                                @if(old('partner_category_id', $partner->partner_category_id) == $partner_category->id) selected @endif
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="protocol" required>
                ...
                @foreach($select_protocols as $protocol)
                    <option value="{{ $protocol }}" @if($protocol == old('protocol', $partner->protocol)){{'selected'}}@endif>{{ $protocol }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="url" value="{{old('url', $partner->url) }}" required>
        </div>
        ...
        <div id="lfm-image">
            @if(old('image', $partner->image))
                <img src="{{ old('image', $partner->image) }}"
