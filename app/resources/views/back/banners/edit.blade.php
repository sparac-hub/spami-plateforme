...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.banners.edit') }}
@endsection

@section('content')

    {!! set_box_head(__('breadcrumbs.back.banners.edit'), false) !!}

    @include('_common.alerts.messages')

    <form action="{{ route('back.banners.update', $banner->id) }}" method="post" class="form-create">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" data-locale="{{$locale}}" name="{{$locale}}[link_type_id]"
                                ...
                                @foreach(config('cms.link_types_banner') as $id => $name)
                                    <option value="{{$id}}" {{ old($locale.'.link_type_id', $banner->translateOrNew($locale)->link_type_id) == $id ? 'selected':'' }} >{{$name}}</option>
                                @endforeach
                            </select>
                        </div>

                        <div id="link_type_{{$locale}}">
                            @if (old($locale.'.link_type_id', $banner->translateOrNew($locale)->link_type_id) == 1)
                                @include('back.banners.form.menu')
                            @elseif (old($locale.'.link_type_id', $banner->translateOrNew($locale)->link_type_id) == 2)
                                @include('back.banners.form.internal_link')
                            @elseif (old($locale.'.link_type_id', $banner->translateOrNew($locale)->link_type_id) == 3)
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="css_class"
                   value="{{ old('css_class') ?? $banner->css_class }}">
        </div>

        {{-- <div class="form-group">
             <label>{{__('og.banners.is_for_mobile')}}</label>
             <select class="form-control" name="is_for_mobile">
                 <option value="0" @if(!(old('is_for_mobile') ?? $banner->is_for_mobile)) selected @endif>Non</option>
                 <option value="1" @if((old('is_for_mobile') ?? $banner->is_for_mobile)) selected @endif>Oui</option>
             </select>
         </div>--}}
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="is_active">
                <option value="0" @if(!(old('is_active') ?? $banner->is_active)) selected @endif>Non</option>
                <option value="1" @if((old('is_active') ?? $banner->is_active)) selected @endif>Oui</option>
            </select>
        </div>

        <div class="form-group">
            <label>{{__('og.banners.width')}}</label>
            <input type="text" class="form-control" name="width" value="{{ old('width') ?? $banner->width }}">
        </div>

        <div class="form-group">
            <label>{{__('og.banners.height')}}</label>
            <input type="text" class="form-control" name="height" value="{{ old('height') ?? $banner->height }}">
        </div>

        {{--
        <div class="form-group">
            <label>{{__('og.banners.thumb')}}</label>
            <input type="text" class="form-control" name="thumb" value="{{ old('thumb') ?? $banner->thumb }}">
        </div>
        --}}
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="type">
                <option value="html"
                        @if((old('type') ?? $banner->type) == \App\Models\Cms\Banner::TYPE_HTML) selected @endif>HTML
                </option>
                <option value="image_file"
                        @if((old('type') ?? $banner->type) == \App\Models\Cms\Banner::TYPE_IMAGE) selected @endif>Image
                    file
                </option>
                <option value="video_file"
                        @if((old('type') ?? $banner->type) == \App\Models\Cms\Banner::TYPE_VIDEO) selected @endif>Video
                    File
                </option>
                <option value="script"
                        @if((old('type') ?? $banner->type) == \App\Models\Cms\Banner::TYPE_SCRIPT) selected @endif>
                    Script
                </option>
            </select>
        </div>

        <div id="image_file" @if((old('type') ?? $banner->type) != "image_file") style="display: none" @endif>
             ...
             <a id="lfm-image-btn" data-input="lfm-image-thumbnail" data-preview="holder" class="btn btn-primary">
               <i class="fa fa-picture-o"></i> {{__('og.banners.image_filepath')}}
                ...
                <input id="lfm-image-thumbnail" class="form-control" type="text" name="image_filepath"
                       value="{{ old('image_filepath')??$banner->image_filepath }}">
            </div>
        ...
        <div id="video_file" @if((old('type') ?? $banner->type) != "video_file") style="display: none" @endif>
            <div class="input-group">
           <span class="input-group-btn">
        ...
        <div id="script" @if((old('type') ?? $banner->type) != "script") style="display: none" @endif>
            <div class="form-group">
                <label>{{__('og.banners.script')}}</label>
                <textarea class="form-control" name="script" rows="3">{{ old('script', $banner->script) }}</textarea>
            </div>
        </div>
        ...
        $(document).ready(function () {
            ...
            $('select[id^="link_type_id"]').on('change', function () {
                ...
                $.ajax({
                    ...
                    data: {link_type_id: link_type_id, local: local, id: id},
                    //dataType: "json",
