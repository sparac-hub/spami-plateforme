...
@section('breadcrumbs')
    {{ Breadcrumbs::render('back.faq_categories.create', $menu_id) }}
@endsection

@section('content')

    {!! set_box_head(__('breadcrumbs.back.faq_categories.create'), false) !!}

    @include('back._common.alerts.messages')

    <form action="{{ route('back.faq_categories.store') }}" method="post" class="form-create">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" name="order" {{ old('order') }}>
        </div>
