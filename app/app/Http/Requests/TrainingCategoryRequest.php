...
class TrainingCategoryRequest extends FormRequest
{
    ...
    {
        $id = $this->route()->parameter('training_category');
        ...
        foreach (config('translatable.locales') as $k => $locale) {
            ...
            if (request()->method() == 'POST') {
                $rules[$locale . '.slug'] = 'required|unique:' . (new TrainingCategoryTranslation())->getTable() . ',slug';
            } else {
                $rules[$locale . '.slug'] = 'required|unique:' . (new TrainingCategoryTranslation())->getTable() . ',slug,' . $id . ',training_category_id';
            }
        }
